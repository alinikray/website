...
import { fetchDiscoverMovies } from '../lib/tmdbService';
...
export default function MoviesPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetchDiscoverMovies(1).then(({ results, totalPages: tp }) => {
      setMovies(results);
  ...
  const loadMore = useCallback(() => {
    ...
    setLoadingMore(true);
    fetchDiscoverMovies(nextPage).then(({ results }) => {
    ...
    <div className="min-h-screen pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="pt-4 pb-6">
          ...
          <p className="text-gray-500">Newest releases first · {movies.length} loaded</p>
        </div>
        ...
        <div ref={sentinelRef} className="flex items-center justify-center py-10">
          ...
          {!hasMore && movies.length > 0 && (
            <p className="text-gray-600 text-sm">All {movies.length} movies loaded</p>
          )}
        </div>
      </div>
    </div>
