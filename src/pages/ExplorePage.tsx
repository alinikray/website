...
import {
  ...
  MessageCircle, ThumbsUp, Send, ChevronsUp
} from 'lucide-react';
...
export default function ExplorePage() {
  ...
  // Auto-hide speaker icon after 3 seconds on mount and on clip change
  useEffect(() => {
    setShowSpeaker(true);
    if (speakerTimer.current) clearTimeout(speakerTimer.current);
    speakerTimer.current = setTimeout(() => setShowSpeaker(false), 3000);
    return () => { if (speakerTimer.current) clearTimeout(speakerTimer.current); };
  }, [currentIndex]);

  // Show speaker on mute toggle
      ...
      <AnimatePresence mode="wait">
        {currentClip && (
          ...
          >
            ...
            {/* SWIPE UP HINT — first interaction only */}
            <AnimatePresence>
              {showSwipeHint && currentIndex === 0 && (
                ...
                >
                  ...
                  >
                    <ChevronsUp className="w-6 h-6 text-white/60" />
                    <span className="text-white/60 text-xs font-medium tracking-wider">Swipe Up</span>
                  </motion.div>
                </motion.div>
              )}
            </AnimatePresence>
